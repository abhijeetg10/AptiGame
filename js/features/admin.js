import { collection, getDocs, doc, getDoc, updateDoc, deleteDoc, query, where, orderBy, limit, auth, onAuthStateChanged, signOut, db, serverTimestamp, Timestamp, getCountFromServer, writeBatch } from "../js/core/db-shim.js";
...
async function fetchMockResults() {
    try {
        ...
        docs.forEach(data => {
            ...
            const feedback = data.feedback || data.comment || "";
            ...
            tr.innerHTML = `
                ...
                <td style="max-width: 200px;">
                    ${rating ? `<div style="color:#fbbf24; font-size:0.75rem;">${"★".repeat(rating)}</div>` : ""}
                    <div style="font-size:0.8rem; color:var(--admin-text-muted);">${feedback || "No feedback"}</div>
                </td>
            `;
            tbody.appendChild(tr);
...
async function filterUsersByCollege(collegeName) {
    ...
    if (searchInput) {
        ...
        // Trigger a fake input event to trigger existing search logic in js/features/admin.js
